...
type Item = { groupId: string; groupTitle: string; tile: { id: string; title: string; url: string; favorite?: boolean } };
...
function Row({ groupId, tileId, edit }: { groupId: string; tileId: string; edit: boolean }) {
  ...
  return (
    <div className="list-row">
      ...
      {edit && (
        <div>
          <button type="button" className="icon-btn" title={tile.favorite ? "Quitar de favoritos" : "Agregar a favoritos"} onClick={() => updateTile(groupId, tile.id, { favorite: !tile.favorite })}>☆</button>
          <button type="button" className="icon-btn" aria-label="Eliminar" title="Eliminar" onClick={() => removeTile(groupId, tile.id)}>🗑️</button>
        </div>
      )}
    </div>
  );
...
async function ping(url: string): Promise<"online" | "offline" | "unknown"> {
  try {
    ...
    clearTimeout(t);
    return "online"; // si responde sin error, consideramos online
  } catch {
    return "offline";
  }
}
